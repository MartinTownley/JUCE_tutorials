...
{//scope begins
    ...
    // we then create a unique pointer to an xml element
    std::unique_ptr<juce::XmlElement> xml (state.createXml());
    //inside the bracket is what we're initialising it to, so the data that is occupying the memory.
    
    copyXmlToBinary(*xml, destData);
    //retrieve the data pointed to by xml,
